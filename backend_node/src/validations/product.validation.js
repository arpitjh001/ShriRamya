...
const createProduct = {
  body: Joi.object().keys({
    ...
    regular_price: Joi.alternatives().try(Joi.string(), Joi.number()).required(),
    sale_price: Joi.alternatives().try(Joi.string(), Joi.number()).optional(),
    stock_quantity: Joi.number().min(0).optional(),
    sku: Joi.string().allow('').optional(),
    status: Joi.string().valid('publish', 'draft', 'pending', 'private').optional().default('publish'),
    ...
    size_stock: Joi.array().items(Joi.object({
      size: Joi.string().required(),
      qty: Joi.number().min(0).required()
    })).optional(),
    color_stock: Joi.array().items(Joi.object({
