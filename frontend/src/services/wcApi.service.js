/**
 * Native Product API Client
 * Manages products, orders, customers, coupons via the backend REST API
 */
...
export const wcProductsAPI = {
  getAll: async (params = {}) => {
    try {
      ...
      return { products: Array.isArray(productsData) ? productsData : [] };
    } catch (err) {
...
export const wcCategoriesAPI = {
  getAll: async () => {
    try {
      ...
      return { categories: Array.isArray(catData) ? catData : [] };
    } catch (err) {
